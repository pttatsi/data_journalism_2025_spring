...
```{r}
pgc_overdose <- read_csv("data/prince_georges_2022_overdoses.csv")
```


```{r}
glimpse(pgc_overdose)
```
```{r}
pgc_overdose <- pgc_overdose |>
  mutate(
...
** I used month because I have a column with the exact dates already, it makes it easier for me to read the graph/ work on this lab. 

**A1.** December accounted for the largest share of total calls in PG County (10.09%), which is higher than Baltimore County, where August had the highest share at 9.58%. 

```{r}
pgc_month <- pgc_overdose |>
  group_by(month) |>
  summarise(count = n()) |>
   mutate(
...
pgc_month |>
  ...
  theme_minimal() +
  labs(
    title= "Monthly Percent Change for PG County Overdose Calls",
    x = "month",
    y = "percent change",
...
+ 
  labs(
    title="Weekly Percent Change for PG County Overdose Calls",
    x = "week",
    y = "percent change",
    caption = "source: Prince George's County EMS "
  )
