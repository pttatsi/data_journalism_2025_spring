---
title: "lab_03"
author: "Derek Willis"
date: "2025-01-16"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## TASK: Load libraries

```{r}
# Turn off scientific notation
options(scipen=999)

# Load the tidyverse
library(tidyverse)
# Load janitor
library(janitor)
library(lubridate)
```

## TASK: Load data and clean up the column names

Load the two datasets we worked with in the pre_lab that are in the data folder: the Maryland payments to vendors and Maryland grants and loans AND, using janitor, change the column names so that they are lower-cased and any spaces are replaced with underscores.

```{r}
# Load in the Maryland state agency payments data, making sure that it correctly sets the datatypes using all of the rows
payments <- read_csv("data/State_of_Maryland_Payments_Data__FY2008_to_FY2024.csv", guess_max=369008) 
```


```{r}
cleaned_payments <- payments |>
  clean_names() |>
mutate(agency_name = str_to_upper(agency_name ), vendor_name = str_to_upper(vendor_name), category = str_to_upper(category)) |>
glimpse()
```



```{r}
# Load in the Maryland grants and loan data
md_grants_loans <- read_csv("data/State_of_Maryland_Grant_and_Loan_Data__FY2009_to_FY2022.csv", guess_max=19482)
```
```{r}
cleaned_md_grants_loans <- md_grants_loans |>
  clean_names() |>
 mutate(grantor = str_to_upper(grantor), grantee = str_to_upper(grantee), description = str_to_upper(description)) |>
 mutate(zip5 = str_sub(zip_code, start=1L, end=5L))
```


## Answer questions

Q1: You want to know how much money state agencies have paid to the Prince George's County School system, but to do that you'll need to find all of the variations of how the school system might be listed in the payments data. Look at & search the records in the dataframe to start your search, starting with a broad search and then narrowing it down. Then write code to find rows where the vendor name contains some version of the county's school system.

HINT: you should use filter() just once, and inside your filter you want to look for the variations one at a time. Think "this version OR that version."

When you have found as many as possible, save the result to a new dataframe called `pg_schools`. How many records did you find?

A1: I found 109 rows related to Prince George's County School system in the vendor name column

```{r}
pg_schools <- cleaned_payments |>
filter(str_detect(vendor_name, "PRINCE GEORGE'S COMM COLLG|PRINCE GEORGES COUNTY PUBLIC SCHOOLS|PRINCE GEORGE'S COMMUNITY COLLEGE|PRINCE GEORGES COMM COLLEGE"))

pg_schools
```

Q2: Using that `pg_schools` dataframe, write code to calculate the total amount of payments to the Prince George's County School system for each fiscal year. Which fiscal year had the largest total amount? Are there any aspects of the results that stand out to you?

A2: The year 2008 had the largest total amount of payments to PG county school system. The range of numbers stands out to me, why did the total reach 1 billion for three years straight? Why did 2023 recieve such little payment compared to the rest? These are extreme outliers 

```{r}
pg_schools |>
group_by(fiscal_year)|>
summarise(yearly_amount = sum(amount, na.rm = TRUE)) |>
arrange(desc(yearly_amount))
```

Q3. You're interested in examining the vendors paid for capital projects at the University of Maryland. Search the payments data to find those payments, then write code to produce a list of each vendor and the total amount it received.

Look carefully at that list - what cleaning will it require to ensure that you have only one row per vendor and zip code? Provide at least three examples of vendor names that are similar and should be combined into one. Describe in words how you might clean that data so that any nearly identical vendors would look identical - what would you change and how?

A3. Use a combination of string functions (str_detect, str_remove_all, str_squish, str_to_upper) to fix punctuation (commas, periods), extra spaces, and capitalization differences so that similar vendors like DEMOLITION SERVICES, INC = DEMOLITION SERVICES. Also, WHITING TURNER CONTRACTING CO, categorized under two different zipcodes and names (WHITING TURNER CONTRACTING COMPANY), we could use open refine to make the names the same. Using Open Refine to standardize similar names like PLANO-COUDON LLC/ PLANO-CONDON, LLC. Convert variations like CO, CORP, LLC, INC into a standardized form or remove them if they are not essential for identification.
```{r}
umd_payments <- cleaned_payments |>
filter(agency_name == "CAPITAL PROJECTS- UNIVERSITY OF MARYLAND") |>
group_by(vendor_name, vendor_zip) |>
summarise(total_amount = sum(amount, na.rm = TRUE)) |>
arrange(desc(total_amount))

umd_payments
```

Q4: You want to understand how much state grant money went to stem cell research projects each fiscal year. Search the data to find grants that fit that description. Once you find which column(s) contain the information you want, your code should find just the grants for stem cell research in a single filter. Then calculate the total amount of money for each fiscal year. Write a sentence summarizing how stem cell research funding has changed over time.

A4: In 2010, the state of Maryland granted a total of 7,870,123 in money to stem cell research, a 6,773,598 decrease from the previous year when they donated 14,643,721. For the next 10 years, funding ebbed between 6 to 8 million with multiple outleirs that drag the data distribution in positive and negative ways. 

```{r}
stem_cell_total <- cleaned_md_grants_loans |>
select(grantor, amount, description, fiscal_year) |>
filter(str_detect(description,"STEM CELL RESEARCH GRANT"))
```

```{r}
stem_cell_total |>
group_by(fiscal_year)|>
summarise(yearly_amount = sum(amount, na.rm = TRUE)) |>
arrange(desc(fiscal_year)) 
```

Q5: Using the Maryland grants and loan data, find records where the zip code is the one where the University of Maryland-College Park is located in and produce a list of grantees from that zip code, along with the total amount they received. Write a paragraph explaining how many grantees are there, what you would need to do to clean the results so that you had one record per grantee and which grantees are most interesting to you?

A5: There are 62 grantees located at UMD's zip codes, the highest is the University of Maryland's E-NNOVATION INITIATIVE PROGRAM AND FUND, which was granted 4,156,250.00. The lowest amounts were loans from the state granted to small businesses such as Terrapin Care Center as relief during the pandemic. They all equal 5,000. I would need to use the get_dupes() function to find possible duplicates, then use distinct () to merge any that are exactly the same to have one record per grantee. The most interesting grantees to me are the state's funds to education, especially towards disabled and young people because it is a good investment into the community and trends related to marginalized groups can signal the priorities or values of a government (especially if in a specific time period). There's also grant money being given as initiative for data centers, a hot topic in pg county because of the environmental risks it poses to the surrounding community. 

```{r}
umd_state_grants <- cleaned_md_grants_loans |>
filter(str_detect(zip5,"20742|20740")) |>
select(grantee, zip5, amount, fiscal_year, description, category) |>
arrange(desc(fiscal_year)) 
```


```{r}
umd_state_grants |>
group_by(grantee, description, category)|>
summarise(total_amount = sum(amount, na.rm = TRUE)) |>
arrange(desc(total_amount)) 
```

