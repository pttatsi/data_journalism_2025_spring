...
```{r}
acs_2023 <- load_variables(year = 2023, "acs5", cache = TRUE)
...
**Q1** What is the Maryland jurisdiction with the lowest median age, according to the ACS 2023 5-year estimates? You will need to find the variable for median age first. **A1** Baltimore City has the lowest median age according to the ACS 2023 5-year estimates.
...
variable names for total 18 and up population in acs 
**A3**
...
acs_2023 |>
filter(str_detect(label, "Estimate!!Total:!!18 years and over:")) 
...
```{r}
#pop_18_up <- load_variables(2010, "sf1", cache = TRUE)
allo_05 <- get_acs(
  geography = "state",
  variables = c(x18_over = "B99187_005"),
  year = 2023
)


md_zipcodes <- get_acs(geography="zcta", variables = "B99187_005", year=2022) |>
  select(-NAME)

md_zips_acs <- allo_05 |>
  inner_join(md_zipcodes, by = c("GEOID")) |>
  select(-variable.x, -variable.y, -estimate.y)
```
