...
**Answer** The first line of the warning says to use the function: problems() to detail the errors. It spits out a table that highlights every row with a problem. This table has five columns which are row (the specific row with a problem), col (the column with the error in our original dataset), expected (it expected a character but encountered a number-a dbl), the actual error, and the file location. 
...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** No, it shows no parsing errors when I run this codeblock. This happened  because we set the guess_max to the exact number of rows/observations in the dataset, making R use the entire set for reference when determining data type. 
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer** Vendor Name is chr, amount field is dbl, and the date column is chr. 
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think explains those records? **Answer** There's a total of 553 rows with zipcodes with less than 5 characters. I think this happened because it was maybe recorded worng or there's gaps in the data.
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **Answer** The codeblock asks R to go to the dataset md_grants_loans, then group and show the columns 'Fiscal Year' and Category, then create an internal column displaying the total count per category of each year. What jumps out at me is R identified some categories marked as N/A meaning the administration did not report what kind of payment it was or maybe it was the kind that couldn't be reported. And that there are some years the state gave didn't give out loans. 
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer** I see that the amount paid can range from as low as $60 in 2009 to 23480.22 in 2011. I also see that the rise is not linear with time. 
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **Answer** the clean_names function standardized the column names all into lower case and used an underscore to connect the spaces between words 
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **Answer** First we assigned a new data set to md_grants_loan, then we cleaned/standardized the header names, then we renamed one header from grantor to source, then we capitalized each entry under the columns source, grantee and description in our cleaned_md_grants_loans dataset. 
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer** There are 58 rows of possible duplicates 
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer** it now has 19,453, 29 rows with the same axact information across all fields have been removed. 
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer** Zipcodes like 94107-1242 have been added to column zip5 as 94107, without the four-digit extension
