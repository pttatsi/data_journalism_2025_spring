...
```{r echo=FALSE}
#census_api_key("bf523cacbe0c8b02fe849fb1312a6a21c4124e7c", install=TRUE)
```
```{r}
...
**Task** Run the following codeblock to calculate the percentage change and put the state with the largest percentage growth below. **Answer** State with the largest percentage growth is nevada 
...
**Task** Run the following codeblock to find the Maryland county with the highest median household income and put the answer below. **Answer** Maryland's richest county is Howard county with a median household income of 129,549
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Answer** Median household income by county in Maryland, 2021
...
Is the difference between Calvert and Montgomery significant? **Answer** no there's not
...
**Answer** Is there a strong relationship/correlation between household income in the high school’s ZIP code and the school’s enrollment percentage? But even if higher income areas have different enrollment percentages, that doesn’t prove income causes those differences as other factors (like school quality, district boundaries, or demographics) could be confounding variables.
...
**Answer** Based on the data, the average household income for high schools where 60% or more graduates immediately enroll in college is $69,252.24 with 76 schools in the area. 
...
**Answer** We could ask which county has the highest percentage of high school graduates enrolled in college and compare with median household income to see there is a correlation. 
