...
**Task** Export the CSV file to your pre_lab_05 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** Column names are jam-packed into the rows, the number counts are next to subdivision, its a whole mess lol. The data we want to calculate, such as incident counts per county, is not ready to be worked with.
...
**Task** Load the data without headers by running the codeblock below. What problems remain? **Answer** We need to rename our column names, they are not clearly defined. 
...
**Task** Run the codeblock below. What else do we need to do? **Answer** We need to remove the periods from the jurisdiction column. Or else it will cause issues later on. 
