...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer** Each column has identifying information for each row which are zip codes. There's a new column called geometry whose 
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** It contains geogrpahical information for maryland zipcodes such as the zips length and spread. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The output is the map of Maryland and we can see each zip code outlined on a longitude-latitude grid
...
**Task** Run the following code. Describe the output in the space below. **Answer** It says "To enable caching of data, set `options(tigris_use_cache = TRUE)`
...
**Task** Run the following code. Describe the output in the space below. **Answer** Its an almost normal dataframe with information on Maryland counties, like md_zips, it also use a clickable geometry column. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** the output is the exact same as the previous question. The only difference is that now we've filtered for only maryland counties
...
**Task** Run the following code. Describe the output in the space below. **Answer** We have clear boundaries for each Maryland county on the map
...
**Task** Run the following code. Describe the output in the space below. **Answer** We overlayed zip codes onto county data, you can kinda see the county map under the zip map, the chesapeake bay looks like a salamandar. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The output is total count of foreclosures in 2023 by maryland zip 
...
**Task** Run the following code. Describe the output in the space below. **Answer** we have joined our md_zips with the foreclosure_zip data so we can now visualize the foreclosures on a map
...
**Task** Run the following code. Describe the output in the space below. **Answer** The range of blue coloring on the map helps to quickly identify the parts with highest foreclosures 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We used only the first row of our dataset to create a new dataframefor Notice of Intent to Foreclose on 03/01/2023 at 12:00:00 AM. Then pivoted the md_county_notices dataframe to make it easier to read. The second output is a dataframe containing population stats for each md county from census data

```{r}
md_county_notices_comparison <- read_csv("data/Maryland_Foreclosure_Data_by_County.csv")
```
...
**Task** Run the following code. Describe the output in the space below. **Answer** We added a geometry column to our md_county_population. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** It showed md_county_notices but we have two type columns 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We now have three columns: county, population and geometry that we can join with md_county_notices
...
**Task** Run the following code. Describe the output in the space below. **Answer** we have count of population and of foreclosures per county in one dataframe 
...
**Task** Run the following code. Describe the output in the space below. **Answer** There is a new column for rates in our dataset 

```{r}
md_pop_with_foreclosures <- md_county_population |>
  #why not just call md_pop_with_foreclosures??
  left_join(md_county_notices, join_by(county)) |> 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The map has names by county and is shaded degrees of blue to reflect the rate of foreclosures 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The map still shows the rate per county but now we have distinguishable colors of purple, pink, cream and black.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** West and East Maryland had the highest amount of foreclosures in 2023. As a resident of Bowie, this is probably partially caused by high real estate development in PG county, especially Bowie and Upper Marlboro. But overall, Maryland saw foreclosures in nearly every region, with the least in the Piedmont region. 
