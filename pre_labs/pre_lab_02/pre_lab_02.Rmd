...
```{r}
research_linguistics <- umd_courses |>
  filter(department == "Linguistics") 

selected_research_linguistics <- research_linguistics |> 
  select("Michelle Morrison")
```
...
**Answer** It is important to isolate the specific data we want to analyze into a new variable to target specific stories and points of interest. Plus, it makes it easier to expand on our set with various calculations such as mean, median, and percentage change.
...
**Answer:** It made the date a date again
...
**Answer**  floor_date rounds up to the nearest boundary of the specified unit, which in this case, was the very first day of the month. then it outputted those dates based on the total sum amount arranged in ascending order. 
...
**Answer** Two new columns, one with Wes Moore's percentage of votes per county, and another with the total number of votes per county, were added to the original data set 
...
**Answer** Now I can see where Moore got the most amount of votes because the function arrange(desc(pct_moore)) ordered the set to descend. 
...
**Answer** yes because it shows the counties he got the least support because the arrange function automatically ascends data. And Moore's adversary in these areas is Cox. 
...
**Task** Search the Internet for tidyverse functions similar to x. Name at least two and describe what they do. If you use ChatGPT for this, include the question you submitted.

**Answer** str_detect(x, ""): takes a string and pattern to search for, if found it responds with TRUE and FALSE for vice versa; str_replace(x, e, ***), replaces identified letters in strings 
...
**Answer**  i think the mutate statement is going through addresses to find specified state names to create a new column with those names abbreviated
...
**Answer** Yes, a new column has been created to summarize the amount donated to each state
...
1.  Are there vendors who consistently receive large payments from the state, and do those payments cluster at certain times of year

2.  What are the different banks used by the state?

3.  how many and what kind of expenditures are being made using only cash?
