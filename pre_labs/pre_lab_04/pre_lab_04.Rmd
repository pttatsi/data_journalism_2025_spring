...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** Mixed capitalization is one probelm that hinders proper grouping of the data, another is punctuation. 
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer** Honestly, I cannot see much difference in the columns from the code I ran. I didn't change anything and cross-referenced with the online textbook. A chnage i observed is that some entries like Melwood Horticultural Training Center, Inc. changed to Melwood Horticultural Training Center, but that's not the same with Take Charge Juvenile Diversion Program, Inc. where "Inc." remains across both columns. 
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer** It's the same thing here, I do not observe any significant changes in the data. 
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** No it does not vary sustainably, I am not sure what the change is. The little changes I see don't make sense to me like one entry being made lower case from all upper case. 
