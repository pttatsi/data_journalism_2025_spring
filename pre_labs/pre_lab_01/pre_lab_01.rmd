...
umd_courses |>
  group_by(department) |>
  summarise(
    count_African_American_Studies = n()
  )
```

Describe what you think each line of this code is doing below, based on the result:

**Answer** translates to: go to umd courses, and then, group the dataset by term, and then, summarise it, and then, count the classes, and then, give me the total number of classes by term 
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** go to umd courses, and then, group data by term, and then, summarise/ count the number of classes per term, and then, arrange the count by descending order:arrange(desc(count_classes))
...
**Answer** The output now has two variables, as opposed to one because we grouped by term and department. Go to umd courses, and then, group by term and department, and then summarise/count the number of classes per term, itemized by department, and then, arrange term and department in ascending order
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** Here we group by term, and then, choose our other variables (count_classes and total sum of seats), and then only sort total of seats by descending order. This way, we can tell which terms offered the most amount of classes and seats. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer** We have used a formula to introduce new informational variables into our data set. Now we have a mean and median of only the total number of seats, not classes. The high difference between the mean and median for our first couple rows means are dataset is positively skewed, not evenly distributed.  
...
**Answer** We have asked R to create new variables such as mean_seats, median (seats), and min_seats, and then R did the calculations for us using the dataset. And then, only arranged the total amount of seats in descending order. We also created variables for highest # of seats and lowest # of seats.
...
1.  which department has a decline in the total amount of classes offered over a five year span

2. who are the top five professors that taught the most amount of classes over a five year span? Is there a correlation by department?

3.  which department has an upcline in the total amount of classes offered over a five year span
