...
**Task** Load the five CSV files. Do they have the same column structure? **Answer** Yes, they have the same exact column structure. 
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There should be 24 rows because we are adding columns not rows.
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **Answer** Joining with `by = join_by(COUNTY)`, exact function from previous codeblock
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** Prince George's county has the highest 18+ Black population as well as highest percentage of Democrats 
...
```{r}
maryland_population_with_voters_and_demographics |>
write_csv("./data/maryland_population_with_voters_and_demographics.csv") 
```
...
**Answer** Howard county has the highest 18+ Asian population with an average percentage of democrats. It seems the 18+ Asian population of 4.6793263 in Prince George's county is minuscule when compared with the 18+ Black population of 60.9035922. However, the total of Howard count's 18+ Asian and Black population does not account for half of the county's demographic so this indicates there's a big group being over looked. 
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **Answer** It is a double or number column
...
It is now a chr or character column
