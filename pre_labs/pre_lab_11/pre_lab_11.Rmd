...
**Task** Run the following code and describe the dataframe it outputs. **Answer** Each row is a disciplinary alert from the Maryland Board of Physicians. The contents of the text column in the dataframe is a summary of the misconduct and sanction received. 
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** It combined "dog" and "dog" into one word but treated Dog as a unique word, its a very literal function
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** with punctuation removed, unnest_tokens created a row for every unqiue word in the text column
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** The top words are articles and prepositions, which is expected and not interesting 
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** Yes, it's more intersting. High frequency of words such as surrender can indicate that more physicians are surrendering their license's to the Maryland Board of Physicians. 
...
**Task** Run the following code and describe the results. **Answer** We get the number of times the word appears in the text column and the overall percentage of times it occurs. 
...
**Task** Run the following code and describe the results. **Answer** Instead of one word, we have the top 10 two-word phrases from the alert's text column and their percentages. 
...
**Task** Run the following code and describe the results. **Answer** There are more noticeable changes in this data compared to the previous two. Individual 4 and 10 are new interesting two-phrases, I wonder who/what those refers to. Pre-hearing conference is a new phrase, it might indicate change in procedure or preference. Health occ is also consistent as the most reocurring two-phrase from 2021 to 2024 
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** There are things such as crash that are negative and I agree with. But words such as deaf and smelled that are marked negative seem a bit questionable to me, because i don't understand what criteria it used to mark those words as negative. But also, for the most part, it's distinctions are a good place to start analysis 
